...
export async function generateDailyReport(dateStr) {
  ...
  // Drop oldest items (from the front of the chronologically-sorted list) until
  // the total character budget fits within the limit.
  let totalChars = 0;
  let startIdx = 0;
  for (let i = sortedItems.length - 1; i >= 0; i--) {
    ...
    const line = `[${t}] ${item.title ? item.title + ' ' : ''}${item.content}`;
    totalChars += line.length + 1;
    if (totalChars > MAX_NEWS_CHARS) { startIdx = i + 1; break; }
  }
  const droppedCount = startIdx;
  const includedItems = sortedItems.slice(startIdx);
  const newsList = includedItems.map((item, i) => {
    const t = item.time || new Date(item.createdAt).toLocaleTimeString('zh-CN', {
      timeZone: 'Asia/Shanghai', hour: '2-digit', minute: '2-digit',
    });
  ...
  const truncationNote = droppedCount > 0
    ? `\n（注：当日共 ${items.length} 条新闻，因长度限制已省略最早的 ${droppedCount} 条，以下为最近 ${includedCount} 条）`
    : '';
