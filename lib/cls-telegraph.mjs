...
const TELEGRAPH_LIST_URL = 'https://www.cls.cn/nodeapi/telegraphList';
const UPDATE_TELEGRAPH_LIST_URL = 'https://www.cls.cn/nodeapi/updateTelegraphList';
const REFRESH_TELEGRAPH_LIST_URL = 'https://www.cls.cn/nodeapi/refreshTelegraphList';
...
function buildTelegraphRow(item, timestamp) {
  ...
  return {
    ...
    recovery: item?.recovery == null ? null : (item.recovery ? 1 : 0),
    important: isImportantItem(item) ? 1 : 0,
    level: item?.level ? String(item.level) : '',
    level_rank: parseLevelRank(item?.level),
    title: String(item?.title || ''),
    content: String(item?.content || ''),
    shareurl: String(item?.shareurl || ''),
    subjects_json: JSON.stringify(item?.subjects || []),
    tags_json: JSON.stringify(item?.tags || []),
    stocks_json: JSON.stringify(item?.stock_list || []),
...
function buildUpdateUrl() {
  const params = new URLSearchParams({
    ...
    sv: '8.4.6',
    rn: '20',
    lastTime: String(firstCtime()),
    hasFirstVipArticle: '1',
    subscribedColumnIds: '',
  });
  return `${UPDATE_TELEGRAPH_LIST_URL}?${params.toString()}`;
}

function buildRefreshUrl() {
  const params = new URLSearchParams({ lastTime: String(firstCtime()) });
  return `${REFRESH_TELEGRAPH_LIST_URL}?${params.toString()}`;
}
...
function applyUpdatePayload(payload) {
  if (Number(payload?.error || 0) !== 0) {
    throw new Error('updateTelegraphList returned error payload');
  }

  const data = payload?.data || {};
  const rollData = Array.isArray(data?.roll_data) ? data.roll_data : [];
  const updateNum = Number(data?.update_num || 0);
...
function applyRefreshPayload(payload) {
  ...
  const startCtime = Number(payload?.i || 0);
  const endCtime = Number(payload?.a || 0);
  if (!mapping || typeof mapping !== 'object' || Array.isArray(mapping)) {
    throw new Error('refreshTelegraphList returned invalid mapping');
  }
...
async function initializeStateFromApi() {
  ...
  try {
    const payload = await httpGetJson(TELEGRAPH_LIST_URL);
    if (Number(payload?.error || 0) !== 0) {
      throw new Error('telegraphList returned error payload');
    }
    const rollData = Array.isArray(payload?.data?.roll_data) ? payload.data.roll_data : [];
